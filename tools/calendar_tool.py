...
class CreateEventInput(BaseModel):
    ...
    start_time: str = Field(description="Start time in ISO format (e.g., '2025-01-29T14:00:00Z')")
    end_time: str = Field(description="End time in ISO format (e.g., '2025-01-29T15:00:00Z')")
...
class UpdateEventInput(BaseModel):
    ...
    title: Optional[str] = Field(default=None, description="New event title (optional)")
    start_time: Optional[str] = Field(default=None, description="New start time in ISO format (optional)")
    end_time: Optional[str] = Field(default=None, description="New end time in ISO format (optional)")
    description: Optional[str] = Field(default=None, description="New event description (optional)")
    attendees: Optional[List[str]] = Field(default=None, description="New attendee list (optional)")
    location: Optional[str] = Field(default=None, description="New event location (optional)")
    add_meet: bool = Field(default=False, description="Add Google Meet link to existing event")
    timezone: str = Field(default='UTC', description="Timezone for the event")
    user_id: str = Field(description="User ID for authentication")
...
@tool
def create_calendar_event_tool(input_data: CreateEventInput, auth_manager) -> str:
    """
...
@tool
def list_calendar_events_tool(input_data: ListEventsInput, auth_manager) -> str:
    """
...
@tool
def update_calendar_event_tool(input_data: UpdateEventInput, auth_manager) -> str:
    """
...
@tool
def delete_calendar_event_tool(input_data: DeleteEventInput, auth_manager) -> str:
    """
...
@tool
def get_calendar_event_tool(input_data: GetEventInput, auth_manager) -> str:
    """
...
def get_calendar_tools(auth_manager) -> List[BaseTool]:
    """
    Get list of Calendar tools configured with auth manager.
    
    Returns LangChain tools ready for use with ToolNode.
    """
    logger.info("🔧 Creating Calendar tools registry")
    
    # Create tools with auth_manager bound
    tools = [
        create_calendar_event_tool.bind(auth_manager=auth_manager),
        list_calendar_events_tool.bind(auth_manager=auth_manager),
        update_calendar_event_tool.bind(auth_manager=auth_manager),
        delete_calendar_event_tool.bind(auth_manager=auth_manager),
        get_calendar_event_tool.bind(auth_manager=auth_manager)
    ]
